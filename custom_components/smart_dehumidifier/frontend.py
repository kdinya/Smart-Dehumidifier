...
Logic restored from working v1.5.2 (creates/updates Resources entry).
"""
...
async def _async_ensure_resource(hass: HomeAssistant) -> None:
    ...
    if found_url == target:
        _LOGGER.warning("SD card resource OK: %s", target)
        return
